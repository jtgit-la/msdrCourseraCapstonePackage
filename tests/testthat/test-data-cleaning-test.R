...
test_that("geom_timeline works", {
  expect_is(ggplot(eq_location_clean(eq_clean_data(read.delim(system.file("extdata", "signif.txt", package = "msdrCourseraCapstonePackage")))), aes(x = DATE, y = COUNTRY, min_date = as.Date("1900-01-01"), max_date = as.Date("2000-01-01"), size = EQ_PRIMARY, color = TOTAL_DEATHS)) +
